...
def calculate_homography_linear(point_sets,n,epsilon):
    ...
    for i in range(N):
        ...
        if len(list_of_inliers) >= 0.9*len(point_sets):
            break
...
def create_img(img,objs,y_max_list,y_min_list):
    ...
    for i,obj in enumerate(objs):
        ...
        text = str(round(y_max_list[i],2))+';'+str(round(y_min_list[i],2))
...
if __name__ == '__main__':
    video_rec = cv2.VideoCapture('test5.mp4')
    suc = True
    cnt = 0
    k_camera = np.array([[733,4.67,320],[0,736,240],[0,0,1]])
    find_latlong(True)
    model = torch.hub.load('ultralytics/yolov5', 'yolov5s', pretrained=True)
    suc,img1 = video_rec.read()
    #lat1, long1 = find_latlong(False)
    while suc:
        ...
        objects1 = objects1.numpy()
        ##print(objects1)


        #lat2, long2 = find_latlong(False)
        suc,img2 = video_rec.read()
        img_right = img2.copy()
        ...
        objects2 = objects2.numpy()
        # ##print(objects2)
        #dist = find_displacement((lat1,long1),(lat2,long2))
        dist = 1
        ...
        R_t = np.array([[1,0,0,1.2],[0,1,0,0],[0,0,1,0]])
        ...
        img_opb = create_img(img2,objects2,y_max_list,y_min_list)
